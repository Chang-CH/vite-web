---
title: 'React Interview: Implement a rating component'
description: 'Implementing and testing a rating component in React'
tags: ['javascript', 'react', 'UI']
---

# Implement a rating component

Pre screening Online Assessment for a New Graduate Front End role.
...
- Initially I designed this component with the idea that on hover updates the displayed rating, and click saves the current displayed rating.
  This approach does not work as in react testing library, the `click` event does not trigger the `mouseover` event.
- I had originally implemented the parent container to have an onclick to act as a click away feature. In the browser this did not
  affect the components functionality, but in testing the click event bubbled to the parent, and cleared the values after the click.
  Perhaps this was due to the order of events triggering being different in web vs testing.